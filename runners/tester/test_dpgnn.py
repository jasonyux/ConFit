...
def load_model(test_args: ScoreNetworkTestArguments, model_args: ModelArguments):
    ...
    loaded_model_args = DPGNNModelArguments(**model_args_dict)

    model = DPGNNModel(loaded_model_args)
...
def main(
    ...
    eval_speed = evaluate_speed_for_dpgnn(
        model, loaded_model_args, test_args, all_test_data
    )
